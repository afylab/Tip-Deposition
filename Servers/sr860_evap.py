...
class sr860Server(GPIBManagedServer):
    ...
    @setting(139, 'custom_resistance', returns='v')
    def custom_resistance(self, c):
        """ Outputs resistance for the SQUID evaporator setup with 1.05kOhm resistor in series
		"""
        dev = self.selectedDevice(c)
        volt = yield dev.x()
        output = yield dev.sine_out_amplitude()
        ans = volt/(output-volt)*1050
        returnValue(ans)
